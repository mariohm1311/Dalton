...
class MolecularDynamics(Simulation):
    ...
    def _print_energy_header(self):
        ...
        e.write('\n# VANDERWAALSCUTOFF %f' % (self.mol.long_range_cutoff[0]))
        e.write('\n# ELECTROSTATICCUTOFF %f A' % (self.mol.long_range_cutoff[1]))
        e.write('\n# KINETIC_CALC_METHOD %s' % (self.mol.kinetic_calc_method))
        e.write('\n# GRADIENT_CALC_METHOD %s' % (self.mol.gradient_calc_method))   
        e.write('\n#\n# -- ENERGY DATA --\n#')
